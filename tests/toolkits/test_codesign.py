...
from coker.toolkits.codesign import ProblemBuilder, Minimise, norm as codesign_norm
...
def _require_casadi(backend):
    if backend not in {"casadi"}:
        pytest.skip("codesign optimisation requires casadi")


def test_optimisation_zero_input_problem(backend):
    _require_casadi(backend)
...
def test_optimisation_accepts_runtime_parameters(backend):
    _require_casadi(backend)
...
def test_optimisation_norm_helper(backend):
    _require_casadi(backend)
...
def test_optimisation_raises_on_infeasible_solve(backend):
    _require_casadi(backend)
