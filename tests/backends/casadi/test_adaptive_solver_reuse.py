...
def test_adaptive_defects_use_local_grids_and_batched_dynamics(monkeypatch):
    problem = _slow_fast_problem()
    grid_sizes = []
    original_lgr_points = solver_module.lgr_points

    def track_lgr_points(count):
        grid_sizes.append(count)
        return original_lgr_points(count)

    monkeypatch.setattr(solver_module, "lgr_points", track_lgr_points)
    ...
    def track_evaluate(function, inputs):
        if recording["enabled"] and function is problem.system.dxdt:
            defect_evaluations.append(inputs)
        return evaluate(function, inputs)

    monkeypatch.setattr(backend, "evaluate", track_evaluate)
    ...
    assert {degree + 2 for degree in degrees}.issubset(grid_sizes)
    assert len(grid_sizes) == len(set(grid_sizes))
    assert len(defect_evaluations) <= len(solution.path.polys)
