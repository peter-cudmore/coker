"""Dependency-neutral primitives used by unfused Coker lowering.

The public compact lowering remains self-contained.  The unfused lowering
imports these helpers instead of importing ``lowering``, avoiding a
``lowering`` ↔ ``unfused_lowering`` import cycle.
"""
...
def _flatten_constant_rows(value, shape: Tuple[int, ...]) -> List[float]:
    if isinstance(value, BilinearWeights):
        if not value.is_constant:
            raise TypeError("expected a constant bilinear operand")
        value = value.constant.toarray()
