...
def decision_degree(
    ...
    """Return decision-variable degree without recursing through the tape.

    Tapes generated by fused matrix expressions can contain thousands of
    linear nodes.  An explicit post-order walk keeps validation independent
    of Python's recursion limit while retaining the caller's memo table.
    """
    if memo is None:
        memo = {}
    ...
    stack: list[tuple[Tracer, bool]] = [(tracer, False)]
    while stack:
        ...
        if isinstance(node, Tracer):
            memo[index] = 1 if index in decision_indices else 0
            continue
        op, *arguments = node
        dependencies = [
            argument
            for argument in arguments
            if isinstance(argument, Tracer) and argument.tape is tape
        ]
        if not expanded:
            ...
            stack.extend(
                ...
                if dependency.index not in memo
            )
            continue
        if op == OP.VALUE:
