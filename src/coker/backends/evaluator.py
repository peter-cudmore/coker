...
def evaluate_inner(graph, args, outputs, backend: Backend, workspace: dict):
    for index, arg in zip(graph.input_indicies, args):
        workspace[index] = backend.from_native(arg)
    ...
    outputs = [
        np.reshape(backend.to_native(workspace[o.index]), o.shape)
        if not o.dim.is_scalar() else backend.to_native(workspace[o.index])
        for o in outputs
    ]
    return outputs
