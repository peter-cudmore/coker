...
class DynamicsSpec:
    ...
    initial_conditions: Callable[[float,VectorSpace, VectorSpace], Tuple[np.ndarray, np.ndarray]]
    """ [x, z] = initial_conditions(t_0, p) """

    dynamics: Callable[[float, np.ndarray, np.ndarray, np.ndarray, np.ndarray], np.ndarray]
    """dx = dynamics(t, x, z, u, p)"""

    constraints: Callable[[float, np.ndarray, np.ndarray, np.ndarray, np.ndarray], np.ndarray]
    """ g(t, x, z, u, p) = constraints(t, x, z, u, p) = 0."""

    outputs: Callable[[float, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray], np.ndarray]
    """ y(t) = outputs(t, x, z, u, p, q) """

    quadratures: Optional[Callable[[float, np.ndarray, np.ndarray, np.ndarray], np.ndarray]] = None
...
class DynamicalSystem:
    def __init__(self,
        ...
        x0: Function, dxdt: Function, g: Optional[Function], dqdt: Optional[Function],
        y: Function
    ):
        self.inputs = inputs
    ...
    def __call__(self, t: float, u: Callable[[float], np.ndarray], p: np.ndarray):
...
def create_dynamics_from_spec(spec: DynamicsSpec, backend=None):
    ...
    assert all(x0.dim == dx.dim for x0, dx in zip(x0.output, xdot.output)), f"Initial conditions and dynamics have different output dimensions"

    constraint = function(arguments, spec.constraints, backend) if spec.algebraic else None
    quadrature = function(arguments, spec.quadratures, backend) if spec.quadratures else None
