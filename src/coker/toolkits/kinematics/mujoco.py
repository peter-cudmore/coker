...
def to_mujoco_xml(body: RigidBody, name: str) -> ET.ElementTree:
    ...
    bodies = []
    joints = []
    for i, (parent_idx, xform, joint, inertia) in enumerate(zip(body.parents, body.transforms, body.joints, body.inertia)):
        ...
        if xform.rotation is not None and xform.rotation != Rotation3.zero():
            orientation = " ".join([f"{q}" for q in xform.rotation.as_quaternion().to_elements()])
        ...
        for axis in joint.axes:
            ...
            if (axis.translation == np.array([0, 0, 0])).all():
                ...
                joint_el.attrib.update(
                    dict(name=f"q_{j}",axis=rot)
            )
            else:
                raise NotImplementedError
        ...
        geom.attrib.update(dict(type='capsule', size='0.01', fromto='0 0 0 ' + to_string(xform.apply(np.zeros((3,))))))
