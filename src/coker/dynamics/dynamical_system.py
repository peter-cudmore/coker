...
def create_dynamics_from_spec(spec: DynamicsSpec, backend="numpy"):
    ...
    assert len(x0.output) == 2, "Initial conditions must a pair, one for the state and one for the algebraic variables"
    ...
    if algebraic is not None:
        assert algebraic.dim == spec.algebraic.dim, "Initial algebraic conditions must have the same dimension as the algebraic variables"
    ...
    if spec.algebraic is not None:
        assert spec.constraints is not Noop(), "If algebraic constraints are specified, constraints must be specified"
...
def create_control_system(
    ...
    if p_init is not None:
        assert parameters is not None, "Parameters must be specified if p_init is specified"
        if isinstance(parameters, Scalar):
            assert isinstance(p_init, (float, int)) or p_init.shape == (1,), "p_init must be a scalar or a 1D array"
            p_init = np.array([p_init])
        else:
            assert p_init.shape == (parameters.dimension, ), "p_init must be a 1D array of the same size as the parameters"
    else:
    ...
    if u_init is not None:
        ...
        u0 = u_init(0)
        assert u0.shape == u_dim.shape, f"u0 must have the same shape as the control output; u0 is {u0} and control output is {u_dim}"
    else:
    ...
    assert ((is_scalar(x0_eval) and is_scalar(dot_x_eval))
...
def create_homogenous_ode(
    ...
    dot_x_eval = xdot(x0_eval, None, None, p_init)
