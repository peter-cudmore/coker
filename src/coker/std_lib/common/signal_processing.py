from coker.modelling import Block, BlockSpec, Signal, Vector,Parameter, Variable, ValueType
...
class Step(Block):
    ...
    def __init__(self, name, parent, t_step=None):
        if t_step is None:
            ...
            super().__init__(name, parent, BlockSpec(outputs=[Signal('y')], parameters=[Parameter('t_step', t_step)]))
            self.t_step = t_step
