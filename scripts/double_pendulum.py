import numpy as np
from coker.toolkits.kinematics import RigidBody, Revolute, Inertia, KinematicsVisualiser
...
block_inertia = Inertia(
    centre_of_mass=Isometry3(translation=np.array([0.0, 0, -0.5])),    # 0.5 m in y
    mass=1,                                                         # 1kg
...
n = 50
q_sweep = [
    i * np.array([np.pi, 0]) / n for i in range(-n, n + 1)
] + [
